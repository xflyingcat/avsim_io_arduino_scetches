...
#define BITS_MAXIMUM 108   /* Регулярные входы */
#define BITS_MAXIMUM_FAST 20  /* количество линий энкодеров */

#if (BITS_MAXIMUM + BITS_MAXIMUM_FAST) != 128
#error  Check defenitions for the encoder lines number
#endif
...
void discrete_task_fast( void);
void discrete_task_slow( void);
...
BITS_DESCRIPTION  fbits[BITS_MAXIMUM_FAST];
...
void discrete_task_fast( void)
{
    ...
    for(i=0;i<BITS_MAXIMUM_FAST;i++)
    {
           bit_state = get_bit(i+BITS_MAXIMUM);
              
           flag = fbits[i].flag;
           bit = FLAG_INVERTED ^ bit_state;
           if(bit)
           {
               if(fbits[i].counter < fbits[i].max)
               {
                   if(++(fbits[i].counter) >= fbits[i].max)
                   {
                       if(fbits[i].signal == 0)
                       {
                           //front __0__/--1--
                           if(fbits[i].mode == ENCODER_A)
                           {
                               send_event(0x2000, fbits[i].id + fbits[i+1].signal);
                           }
                        }
                        fbits[i].signal = 1;
                    }
                ...
                if(fbits[i].counter > 0)
                {
                    if(--(fbits[i].counter) <= 0)
                    {
                        if(fbits[i].signal)
                        {
                           ...
                           if(fbits[i].flag & FLAG_HALF_CYCLE_EC)
                           {
                               if(fbits[i].mode == ENCODER_B)
                               {
                                   send_event(0x2000, fbits[i-1].id + fbits[i-1].signal);
                               }
                           } 
                        }   
                        fbits[i].signal = 0;
...
fbits[x-1].signal = 0;\
fbits[x-1].max = 2;\
fbits[x-1].flag = (FLAG_ENABLED | FLAG_INVERTED );\
fbits[x-1].mode = m;\
fbits[x-1].id = x + BITS_MAXIMUM;\
} while(0)
...
fbits[x-1].signal = 0;\
fbits[x-1].max = 2;\
fbits[x-1].flag = (FLAG_ENABLED | FLAG_INVERTED | FLAG_HALF_CYCLE_EC);\
fbits[x-1].mode = m;\
fbits[x-1].id = x + BITS_MAXIMUM;\
} while(0)
...
{  
    ...
    /*          Энкодеры            */
    /********************************/
    for(i=1;i<BITS_MAXIMUM_FAST; i+=2)
    {
       input_init_fast(i,ENCODER_A); 
       input_init_fast(i+1,ENCODER_B); 
    }


    /* half cycle encoder */
      //input_init_fast_half_cycle_ec(40,ENCODER_A); 
      //input_init_fast_half_cycle_ec(41,ENCODER_B);
